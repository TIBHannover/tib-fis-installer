	...
	<section id="individual-info" ${infoClass!} role="region">
		<#if editable>
			<#if claimSources?size &gt; 0>
				...
				<form id="claimExisting" action="${urls.base}/createAndLink/claim" method="get" style="display: none;">
					<input type="hidden" name="profileUri" value="${individual.uri}" />
					<input type="submit" class="submit" value="${i18n().claim_publications_existing}" />
				</form>
				<script>
					$.getJSON("${urls.base}/createAndLink?suggestionsForUri=${individual.uri}", function(json) {
						if (json.length > 0) {
							$("#claimExisting").css("display", "inline-block");
						}
					});
				</script>
			</#if>
...
<div class="row">
	...
	<div id="ind-right-panel" class="col-md-4">
